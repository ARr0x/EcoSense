...
**Failover Route 53 et Lambda health check** — Route 53 health checks ne peuvent surveiller que des endpoints HTTPS standard (port 443, réponse HTTP 200). L'endpoint IoT Core (`*.iot.amazonaws.com`) est un broker MQTT/TLS sur le port 8883 — Route 53 ne peut pas lui parler. La Lambda `ecosense-health-{region}` (`iac/stacks/health_stack.py`) est donc un **proxy de santé** : une Function URL HTTPS publique qui répond simplement `{"status": "ok", "region": "..."}`. Route 53 surveille cette URL ; si la région tombe, la Lambda ne répond plus, Route 53 passe le health check en `UNHEALTHY`, et le simulateur détecte ce changement via `boto3` (polling toutes les N salves) pour basculer proactivement sur la région secondaire avant toute déconnexion MQTT. Variables requises dans `.env` : `ROUTE53_HC_ID_PRIMARY`, `ROUTE53_HC_ID_SECONDARY`. Si absentes, seul le failover réactif MQTT est actif.
